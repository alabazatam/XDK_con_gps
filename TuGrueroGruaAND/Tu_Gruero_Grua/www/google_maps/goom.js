...
function MakeRuta() {
	if (typeof google === 'object' && typeof google.maps === 'object') {
		...
		if (mapas.mapaRuta === null) {
			mapas.mapaRuta = makeMapa("maparuta", gpsMarcadores.Grua);
		} else {
			google.maps.event.trigger(mapas.mapaRuta, 'resize');
			UbicarEnMapa(mapas.mapaRuta, gpsMarcadores.Grua);
		}
		closePops();
		SetRuta();
	} else {
...
function MakeDestino() {
	if (typeof google === 'object' && typeof google.maps === 'object') {
		...
		if (mapas.mapaDestino === null) {
			mapas.mapaDestino = makeMapa("mapadestino", gpsMarcadores.Grua);
		} else {
			google.maps.event.trigger(mapas.mapaDestino, 'resize');
			UbicarEnMapa(mapas.mapaDestino, gpsMarcadores.Grua);
		}
		closePops();
		SetDestino();
	} else {
...
function refreshGruaPos(mapa) {
	...
	if (mapa === mapas.mapaRuta) {
		markRuta[2].setPosition(gpsMarcadores.Grua);
	} else {
		markDestinos[1].setPosition(gpsMarcadores.Grua);
	}

	UbicarEnMapa(mapa, gpsMarcadores.Grua);
